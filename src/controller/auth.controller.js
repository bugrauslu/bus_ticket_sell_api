...
try {
	...
	if (!userInfo)
		return Response.error401(res,"No user found for this e-mail address");
	const comparePassword = bcrypt.compareSync(password, userInfo.password);
	if (!comparePassword)
		return Response.error400(res,"Email or Password is Incorrect !");

	//token oluşturduğum yer
	const token = await JWT.createToken(email)
	if (!token) 
		return Response.error400("an unexpected error")	
	else
